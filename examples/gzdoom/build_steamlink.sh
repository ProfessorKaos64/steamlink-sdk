...
export TOP="${PWD}"
export DESTDIR="${PWD}/steamlink/apps/gzdoom"
export SRC="${PWD}/gzdoom-src"
GITURL="https://github.com/rheit/zdoom.git"
#OPTS="--recursive"
...
sudo apt-get install -y --force-yes build-essential zlib1g-dev libsdl2-dev libjpeg-dev \
nasm tar libbz2-dev libgtk2.0-dev cmake git libfluildsynth-dev libgme-dev libopenal-dev \
timidity

# FMOD 32 bit

wget -P  "${SRC}" "http://web.archive.org/web/20151111191152/http://doomgod.com/ports/fmod/fmodapi4236linux.tar.gz"
tar -xzvf "${SRC}/fmodapi4236linux.tar.gz" -C "${SRC}"
...
# Needs:
# gc 4.6+, make, cmake 2.8.8+, fmodex, sdl2, zlib (optional), libbzip2 (optional), 
# libjpeg (optional), nasm (optional), mesa, gzmessage, git, fluidsynth (optional),
# libgme (optional), libopenal (optional), timidity (optional).
...
# Strip the binary
# This then may not be needed?

armv7a-cros-linux-gnueabi-strip "${DESTDIR}/gzdoom"
